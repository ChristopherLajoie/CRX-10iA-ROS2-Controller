...
class MoveitController(Node):
    def __init__(self):
        ...
        # Set up timer to process commands
        ...
        for param in params:
            if param.name == 'joint_goals':
                try:
                    ...
                    if not isinstance(joint_goals_deg, list):
                        raise ValueError("joint_goals must be a list of degrees")
                    # Convert degrees to radians
                    self.joint_goals_rad = [math.radians(angle) for angle in joint_goals_deg]
                    self.get_logger().info(f"Updated joint goals to {self.joint_goals_rad} radians")
